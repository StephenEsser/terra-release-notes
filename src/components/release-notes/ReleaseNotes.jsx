...
const ReleaseNotes = () => {
  ...
  const releaseNotes = releases.slice(0, pageCount).map(({ date, notes }) => {
    const dateNotes = notes.map((componentNotes) => (
      <Notes key={`${date}-${componentNotes.component || componentNotes.repo}`} {...componentNotes} />
    ));

    return { date, notes: dateNotes };
  });

  return (
    <div className={cx('release-notes')}>
      <h1 className={cx('title')}>Terra Release Notes</h1>
